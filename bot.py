...
OWNER_ID = 8448843919  # Change to your Telegram ID
VIP_FILE = "vip_data.json"

waiting_for_screenshot = {}  # Tracks users waiting to send payment screenshot
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "Buy VIP":
        ...
        await update.message.reply_text(
            ...
            "and access will be granted automatically.",
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "vip_trc":
        ...
        await query.message.reply_text(
            "<b>After depositing, send the screenshot below. Usually confirmed within 30 min.</b>\n\n"
...
async def add_vip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            chat_id=int(user_id),
            text=f"💎 Your VIP subscription is confirmed! You now have {days} days access."
        )
    except Exception:
        pass
...
async def vip_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not data:
        ...
        return
    report_lines = [f"ID: {uid} | Days left: {(datetime.fromisoformat(exp)-datetime.utcnow()).days}" for uid, exp in data.items()]
    await update.message.reply_text("\n".join(report_lines))
